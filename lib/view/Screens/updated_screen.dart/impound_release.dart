...
class ImpoundRelease extends StatefulWidget {
  const ImpoundRelease({super.key});

  @override
  State<ImpoundRelease> createState() => _ImpoundReleaseState();
}

class _ImpoundReleaseState extends State<ImpoundRelease> {
  ...
  void submitToFirestore() async {
    if (orderNumberController.text.isEmpty ||
        ...
        carDescriptionController.text.isEmpty ||
    ...
    try {
      await FirebaseFirestore.instance.collection('impound_releases').add({
        ...
        'carDescription': carDescriptionController.text.trim(),
        'amount': amountController.text.trim(),
        'timestamp': FieldValue.serverTimestamp(),
      });

      Get.snackbar('Success', 'Impound release saved successfully');
      clearFields();
    } catch (e) {
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(height: mediaQuerySize.height * 0.02),
                    CustomField(
                        text: 'Entry Date', controller: entryDateController),
                    SizedBox(height: mediaQuerySize.height * 0.02),
                    CustomField(
